...
import { action } from "../app/routes/api.twilio-beta";
...
vi.mock("../app/db.server", () => ({
  default: {
    shop: {
      findUnique: vi
        .fn()
        .mockResolvedValue({ id: 1, shop_domain: "test.myshopify.com" }),
    },
...
vi.mock("../app/utils/encryption.server", () => ({
  encryptToken: vi
    .fn()
    .mockReturnValue({ encrypted: "abc", iv: "iv", tag: "tag" }),
...
describe("API WhatsApp Info Endpoint", () => {
  test("debe rechazar token inválido", async () => {
    ...
    const { TokenValidator } = await import(
      "../app/utils/token-validator.server"
    );
    TokenValidator.validateMetaAccessToken = vi
      .fn()
      .mockReturnValue({ valid: false, reason: "Token inválido" });
  ...
  test("debe aceptar token válido", async () => {
    const request = new Request("http://localhost/api/whatsapp-info", {
      ...
      body: JSON.stringify({
        action: "configure_whatsapp",
        shop_domain: "test.myshopify.com",
        metaConfig: { accessToken: "EAA" + "x".repeat(100) },
      }),
    });
    const response = await action({ request } as any);
    const data = await response.json();
